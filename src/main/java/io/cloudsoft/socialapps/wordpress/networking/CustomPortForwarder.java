package io.cloudsoft.socialapps.wordpress.networking;
...
public class CustomPortForwarder implements PortForwarder {

    private static final Logger log = LoggerFactory.getLogger(CustomPortForwarder.class);

    public static final ConfigKey<String> DEFAULT_GATEWAY = ConfigKeys.newStringConfigKey("bt.sla.default.gateway",
    ...
    public CustomPortForwarder(PortForwardManager portForwardManager) {
    ...
    public String openStaticNat(Entity serviceToOpen) {
        ...
        //might be an option to avoid explicit port mapping.
//        String ipAddress = serviceToOpen.getAttribute(Attributes.ADDRESS);
...
//        PublicIPAddress publicIpAddress = client.createIpAddressForVpc(vpcId.get());
//        client.getNATClient().enableStaticNATForVirtualMachine(vm.get().getId(), publicIpAddress.getId());
//        return publicIpAddress.getIPAddress();
    }
...
//        openFirewallPortRange(entity, PortRanges.fromInteger(port), protocol, accessingCidr);
    ...
    public HostAndPort openPortForwarding(HasNetworkAddresses targetVm, int targetPort, Optional<Integer> optionalPublicPort,
        ...
        try {
            synchronized (mutex) {
                Maybe<PublicIPAddress> allocatedPublicIpAddressId = client.findPublicIpAddressFromVmId(vm.get().getId());
