...
public class WordpressSshDriver extends AbstractSoftwareProcessSshDriver implements WordpressDriver {
    ...
    public void install() {
        ...
        commands.add(BashCommands.installPackage(of("yum", "httpd", "apt", "apache2"), null));

        commands.add(alternatives(installPackage(of("yum", "php", "apt", "php5"), null)));
//                installPackage("php")), "php/php53 not available"));

        commands.add(alternatives(Arrays.asList(
                installPackage(of("yum", "php-mysql", "apt", "php-mysql"), null))));
...
//        commands.add(installPackage(of("apt", "libapache2-mod-auth-mysql"), null));
    ...
    public void preInstall() {
        ...
        setExpandedInstallDir(Os.mergePaths(getInstallDir(), resolver.getUnpackedDirectoryName("wordpress")));
    }
    ...
    public void addW3TotalCache() {
        try {
            ...
            newScript("installing plugin " + name).
                    failOnNonZeroResultCode().
                    body.append("cd /tmp").
                    body.append(BashCommands.commandsToDownloadUrlsAs(Arrays.asList(url), filename)).
